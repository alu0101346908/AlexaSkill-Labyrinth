...
function ManageDirection (AnswerValue, CurrentWorld, player_position_package){
 ...
 function Surroundings(CurrentWorld, player){
    ...
    let y = player.player_pointer_y;
        ...
        case 'N':
            if (x-1 > 0) behind = CurrentWorld[x-1][y][0];
            else behind = 'X';
            if (x+1 < CurrentWorld.length) front = CurrentWorld[x+1][y][0];
            else front = 'X';
            if (y-1 > 0) left = CurrentWorld[x][y-1][0];
            else left = 'X';
            if (y+1 < CurrentWorld.length) right = CurrentWorld[x][y+1][0];
            else right = 'X';
            break;    
        case 'S':
            if (x-1 > 0) front = CurrentWorld[x-1][y][0];
            else front = 'X';
            if (x+1 < CurrentWorld.length) behind = CurrentWorld[x+1][y][0];
            else behind = 'X';
            if (y-1 > 0) right = CurrentWorld[x][y-1][0];
            else right = 'X';
            if (y+1 < CurrentWorld.length) left = CurrentWorld[x][y+1][0];
            else left = 'X';
            break;
        case 'O':
            if (x-1 > 0) left = CurrentWorld[x-1][y][0];
            else left = 'X';
            if (x+1 < CurrentWorld.length) right = CurrentWorld[x+1][y][0];
            else right = 'X';
            if (y-1 > 0) front = CurrentWorld[x][y-1][0];
            else front = 'X';
            if (y+1 < CurrentWorld.length) behind = CurrentWorld[x][y+1][0];
            else behind = 'X';
            break;
        case 'E':
            if (x-1 > 0) right = CurrentWorld[x-1][y][0];
            else right = 'X';
            if (x+1 < CurrentWorld.length) left = CurrentWorld[x+1][y][0];
            else left = 'X';
            if (y-1 > 0) if (x-1 > 0) behind = CurrentWorld[x][y-1][0];
            else behind = 'X';
            if (y+1 < CurrentWorld.length) front = CurrentWorld[x][y+1][0];
            else front = 'X';
            break;
        }
